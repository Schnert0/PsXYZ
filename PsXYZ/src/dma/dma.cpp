...
void c_dma::getComponents(c_gpu* gpuPtr, c_mem* memPtr){
    gpu = gpuPtr;
    mem = memPtr;
}
...
Uint32 c_dma::read(Uint32 offset){
...
void c_dma::write(Uint32 offset, Uint32 data){
...
void c_dma::setInterrupt(Uint32 data){
    dummy = data & 0x3f;
    forceIrq = (data >> 15) & 1;
...
Uint32 c_dma::transferSize(Uint8 chan){
...
bool c_dma::dmaActive(Uint8 chan){
     bool trigger;
    switch(channel[chan].sync){
...
void c_dma::doBlock(Uint8 chan){
    ...
    while(dataSize > 0){
        ...
        case 0:
            ...
            mem->store(addr, src, WIDTH_WORD);
            break;
        case 1:
            src = mem->load(addr, WIDTH_WORD);
            switch(chan){
...
void c_dma::doLinkedList(Uint8 chan){
    ...
    while(running){
        ...
        if(addr == 0xffffff || addr == 0x000000 || addr == 0x1ffffc)
            running = false;
